...
"""
python generatordiff.py -c /27T/datasets/lunwen/conv-tasnet/code/changedomainloss/config/config.yaml -d /27T/datasets/lunwen/speechdatasets/rowlibrispeech/ -ch /27T/datasets/dataset2019916 -o /27T/datasets/lunwen/speechdatasets/convtasnet/chiengmix/englishifirst
...
def mix(hp, args, num, s1_target, s2, train):  #
    ...
    #w1, w2, mixed = w1/norm, 0.3*w2/norm, mixed/norm  # 这里要给w2乘以0.3吗？
    w1, w2, mixed = 0.3 * w1 / norm, w2 / norm, mixed / norm
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ch', '--chinese_dir', type=str, default=None,
                        help="Directory of Chinese dataset, containing folders of train-clean-100, train-clean-360, train-other-500, dev-clean.")
    ...
    if args.chinese_dir is not None:
        train_folders1 = [x for x in glob.glob(os.path.join(args.chinese_dir, 'train-clean-100', '*'))
                            if os.path.isdir(x)] + \
                        [x for x in glob.glob(os.path.join(args.chinese_dir, 'train-clean-360', '*'))
                            if os.path.isdir(x)] + \
                        [x for x in glob.glob(os.path.join(args.chinese_dir, 'train-other-500', '*'))
                            if os.path.isdir(x)]
        test_folders1 = [x for x in glob.glob(os.path.join(args.chinese_dir, 'dev-clean', '*'))]
    ...
    train_spk1 = [glob.glob(os.path.join(spk, '**', hp.form.input), recursive=True)
                 for spk in train_folders1]
    train_spk1 = [x for x in train_spk1 if len(x) >= 2]
    ...
    test_spk1 = [glob.glob(os.path.join(spk, '**', hp.form.input), recursive=True)
                for spk in test_folders1]
    test_spk1 = [x for x in test_spk1 if len(x) >= 2]


    def train_wrapper(num):
        spk1 = random.sample(train_spk, 1)  # 英文
        spk2 = random.sample(train_spk1, 1)  # 中文
        # s1_dvec, s1_target = random.sample(spk1, 2)
        # print(spk1)
        # print('llllll', len(spk1[0]))
        # print('llllll', len(spk2[0]))
        # raise RuntimeError
        s1 = random.choice(spk1[0])
        s2 = random.choice(spk2[0])
        # print(s1)
        # raise RuntimeError
        mix(hp, args,num, s1, s2, train=True)

    def test_wrapper(num):
        spk1 = random.sample(test_spk, 1)
        spk2 = random.sample(test_spk1, 1)
        s1 = random.choice(spk1)
        s2 = random.choice(spk2)
        mix(hp, args, num, s1, s2, train=False)

    # arr = list(range(int(0.5*(10**5))))
    arr = list(range(int(10 ** 4)))
    with Pool(cpu_num) as p:
        r = list(tqdm.tqdm(p.imap(train_wrapper, arr), total=len(arr)))

    arr = list(range(int(10**3)))
    with Pool(cpu_num) as p:
